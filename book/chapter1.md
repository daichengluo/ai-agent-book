...
它让模型不再只会用文字回答，而是能主动开口说“我要用某个工具”：模型按照约定的格式写清楚用哪个工具、传什么参数（例如“查天气，城市＝北京”），程序照着去执行，再把真实结果交回给模型。这样，模型就从“动嘴说”变成了“能动手做”。本书后续统一使用“工具调用”这一术语。
...
这种能力有两个来源：一是预训练，它让模型见识了海量知识和语言模式；二是后来一种专门“训练模型思考”的方法，让模型逐渐学会在回答前先一步步把问题想清楚（这类擅长思考的模型以 o1、DeepSeek-R1 为代表，背后的训练方法第七章会展开）。模型在思考时并不是随机试错，而是在已有知识基础上进行推演，例如运用数学规律、因果关系和问题分解策略来分析问题。

因此，即使面对从未见过的任务，LLM 往往也能靠已有知识现场推演出解决办法，快速上手新的场景。
...
>
> 用同一个 Agent、同一个任务（“查询三个城市的天气并比较”），每次只从上下文里抽掉一类信息，观察行为如何变化——就像医生逐一排除病因，每次拿掉一个组件，看系统还能不能正常工作，从而判断每个组件的作用。
>
> #### 实验设计
>
> 上下文的五个组件中，系统提示词是 Agent 的身份定义，抽掉它连“自己该扮演什么角色”都不知道，测试没有意义，因此不参与消融。其余四个各抽掉一次，加上完整基线，共五组。图 1-1 列出了每一组分别保留和移除了哪些组件，以及对应的预期行为。
>
> 两点实现说明：其一，“无工具执行结果”不能直接删掉 tool 消息——多数推理栈要求 assistant 的 tool_calls 与后续 tool 消息严格配对，删掉会导致请求非法，正确做法是把结果内容替换成固定占位符。其二，“无思考过程”需要一个会返回思考内容（reasoning_content）的开源思考模型（如 DeepSeek-R1、Qwen3-Thinking），在把上一轮回复写回上下文时滤掉其中的思考部分即可；这在本地部署的开源模型上可以直接实现，无需商用 API（最小工具调用循环见第二章实验 2-1）。
>
...
> 与完整基线相比：
>
> - 无工具定义：Agent 不再调用任何工具，彻底丧失行动能力；
> - 无工具执行结果：Agent 看不到反馈，反复执行相同操作，陷入循环直至超时；
> - 无思考过程：Agent 的决策前后不一致，缺乏连贯的规划；
> - 无历史消息：Agent 重复已经完成的步骤，仿佛“忘记”自己做过什么。
>
> #### 实验结论
>
> Agent 的能力不仅来自模型本身，也来自它能看到的信息。上下文决定了 Agent 能看到什么，而 Agent 只能基于看到的信息做决策——缺失任一组件，行为都会以不同方式退化。这正是上下文在 Agent 系统中不可替代的原因。
