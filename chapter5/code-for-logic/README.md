...
## 真实运行结果（二）：LLM 对照实验（gpt-5.6-luna，12 题）
...
提升(代码辅助 - 纯思考): +0.0 个百分点
```

> 说明：`gpt-5.6-luna` 是具备强推理能力的模型，在这 12 道题上**两种模式都达到了
> 100%**——它足够强，能在“脑内”正确完成多人链式与计数类谜题的真值传播，因此纯思考
> 与代码辅助持平。代码辅助（乃至离线 solver，见上一节 100%）的价值在于把正确性变成
> **确定性、与模型强弱无关**：对能力较弱的模型或更大/更难的谜题，纯思考会随人数增加而
> 掉分，而“翻译成约束程序 + 求解器穷举”的路径始终能稳定给出正确解。`gpt-5.6-luna`
> 具有随机性，多次运行个别题目可能有小幅波动。
...
- **成本**：默认 `gpt-5.6-luna`，跑完 12 题两种模式的开销很小；用 `MODEL`/`--model` 可换更便宜或更强的模型。
